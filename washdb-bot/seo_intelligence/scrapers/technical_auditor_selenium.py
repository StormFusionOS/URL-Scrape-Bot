"""
Technical Auditor Module (SeleniumBase Version)

Performs technical SEO audits on websites using SeleniumBase.
Uses undetected Chrome for anti-detection when auditing external sites.
...
class TechnicalAuditorSelenium(BaseSeleniumScraper):
    """
    Technical SEO auditor using SeleniumBase.
    ...
    def __init__(
        ...
        """
        Initialize technical auditor with SeleniumBase UC drivers.
        ...
        logger.info("TechnicalAuditorSelenium initialized (tier=D)")

    def _http_fallback_audit(self, url: str) -> Optional[Tuple[str, Dict[str, Any], List[AuditIssue]]]:
        """
        Simple HTTP fallback when SeleniumBase fails.
    ...
    def audit_page(self, url: str, measure_cwv: bool = False) -> AuditResult:
        """
        Perform technical audit on a single page using SeleniumBase.

        Args:
            url: URL to audit
            measure_cwv: Whether to measure Core Web Vitals (not supported in Selenium)
        ...
        try:
            ...
            try:
                ...
                logger.warning(f"SeleniumBase error for {url}: {se_error}")
    ...
    def run(self, urls: List[str]) -> Dict[str, Any]:
        ...
        with task_logger.log_task("technical_auditor_selenium", "audit", {"url_count": len(urls)}) as task:
            for url in urls:
...
_technical_auditor_selenium_instance = None


def get_technical_auditor_selenium(**kwargs) -> TechnicalAuditorSelenium:
    """Get or create the singleton TechnicalAuditorSelenium instance."""
    global _technical_auditor_selenium_instance

    if _technical_auditor_selenium_instance is None:
        _technical_auditor_selenium_instance = TechnicalAuditorSelenium(**kwargs)

    return _technical_auditor_selenium_instance
