"""
Generate Bing Local Search city-first scraping targets.

This module generates target lists by expanding:
  state_ids → all cities → all allowed categories

Each target represents a city × category combination to be scraped from Bing Local Search.

Usage:
    python -m scrape_bing.generate_city_targets --states RI CA TX
    python -m scrape_bing.generate_city_targets --states RI --clear
"""
...
from db.models import CityRegistry, BingTarget
...
CATEGORIES_PATH = "scrape_bing/categories.csv"
...
def load_categories(csv_path: str) -> list[dict]:
    ...
    Returns:
        List of dicts with keys: 'label', 'keyword', 'priority'
    """
    categories = []
    ...
    with open(full_path, "r", encoding="utf-8") as f:
        ...
        for row in reader:
            categories.append({
                ...
                "priority": int(row.get("priority", "2").strip()),
            })
...
def generate_targets(session, state_ids: list[str], clear_existing: bool = False) -> int:
    """
    Generate Bing Local Search scraping targets for specified states.
    ...
    3. For each city × category:
       ...
       - Insert into bing_targets table
    ...
    """
    print(f"\n{'='*70}")
    print(f"Bing Local Search Target Generation")
    print(f"{'='*70}")
    ...
    if clear_existing:
        ...
        deleted_count = session.execute(
            delete(BingTarget).where(BingTarget.state_id.in_(state_ids))
        ).rowcount
        session.commit()
    ...
    # Show category breakdown by priority
    priority_1 = sum(1 for c in categories if c.get('priority') == 1)
    priority_2 = sum(1 for c in categories if c.get('priority') == 2)
    priority_3 = sum(1 for c in categories if c.get('priority') == 3)
    print(f"     • Priority 1: {priority_1}")
    print(f"     • Priority 2: {priority_2}")
    print(f"     • Priority 3: {priority_3}\n")
...
def show_target_stats(session, state_ids: list[str] = None):
    ...
    for status_val in ["PLANNED", "IN_PROGRESS", "DONE", "FAILED", "STUCK", "PARKED"]:
        count = query.filter(BingTarget.status == status_val).count()
        if count > 0:
            by_status[status_val] = count

    for priority_val in [1, 2, 3]:
        count = query.filter(BingTarget.priority == priority_val).count()
        if count > 0:
            by_priority[priority_val] = count

    print(f"\n{'='*70}")
    print(f"Bing Local Search Target Statistics")
    print(f"{'='*70}")
    if state_ids:
        print(f"States: {', '.join(state_ids)}")
    print(f"\nTotal targets: {total:,}")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate Bing Local Search city-first scraping targets"
    )
    parser.add_argument(
        "--states",
        type=str,
